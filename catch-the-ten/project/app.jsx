      ...
      <div className="felt-wrap"
        ...
        <div className="table-grid">
          {[0,1,2,3].map(seat => {
            ...
            const tensCaught = matchHistory.reduce((acc, h) => {
              // tens captured per seat — engine doesn't expose; show team's running tens this hand
              return acc;
            }, 0);
            return (
        ...
        {/* Bottom action area */}
        {false && phase === "play" && pendingNext && trick.length < 4 && players[toPlayIdx].kind === "ai" && !aiThinking && !trick.some(t => t.seat === toPlayIdx) && (
          <div style={{ position: "absolute", left: 24, bottom: 28, zIndex: 7 }}>
            <button className="btn primary" onClick={advanceAi}>
              Next ▸ {players[toPlayIdx].name}
            </button>
          </div>
        )}
        {yourTurn && (
